...
class GatsbyImage extends Component {
  ...
  getProps () {
    return {
      ...
      "image": (alt, img, fluid, style, title) => ({
        "alt": alt,
        "height": img.height,
        "onLoad": () => {
      ...
      "proxy": (alt, src, style, title) => ({
        "alt": alt,
        "src": src,
        "style": this.getStyle ().proxy (style),
        "title": title
      }),
      "ratio": (img) => ({
  ...
  img (props) {
    const { style } = props
    const b = this.getStyle ().image (style)
    const c = Object.assign ({}, props, { "style": b })
      ...
      return h (
        ...
        h (
          ...
          img.base64 &&
            h (
              this.img,
              this.getProps ().proxy (alt, img.base64, imgStyle, title)
            ),
          ...
          this.state.isVisible &&
            h (
              this.img,
              this.getProps ().image (alt, img, fluid, imgStyle, title)
            ),
